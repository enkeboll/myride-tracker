...
class MyRideAPIClient:
    ...
    async def get_user_info(self, access_token: str, session: aiohttp.ClientSession | None = None) -> dict[str, Any]:
    ...
    async def get_student_info(
        self, access_token: str, tenant_id: str, session: aiohttp.ClientSession | None = None
    ) -> list[dict[str, Any]]:
        url = f"{self.base_url}/api/student"
        headers = {**DEFAULT_HEADERS, "Authorization": f"Bearer {access_token}", "x-tenant-id": tenant_id}
    ...
    async def negotiate_signalr(
        ...
        params = {"x-tenant-id": tenant_id, "negotiateVersion": "1"}
        headers = {**DEFAULT_HEADERS, "Authorization": f"Bearer {access_token}", "x-tenant-id": tenant_id}
